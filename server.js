...
function updateComment(url, request) {
  ...
  else if (requestComment && requestComment.body && requestComment.username &&
      requestComment.id && requestComment.articleId &&
      (requestComment.id === id) &&
      (requestComment.username === database.comments[id].username) &&
      (requestComment.articleId === database.comments[id].articleId)) {

    database.comments[id].body = requestComment.body
    response.status = 200
    response.body = requestComment.body
  }
  return response
}
...
function deleteComment(url, request) {
  ...
  if (!savedComment) {
    ...
    const inArticleIndex = database.articles[articleId].commentIds.indexOf(id);
    database.articles[articleId].commentIds.splice(inArticleIndex, 1);
    const authorId = database.comments[id].username;
    const inUsercommentsIndex = database.users[authorId].commentIds.indexOf(id);
    database.users[authorId].commentIds.splice(inUsercommentsIndex, 1);
...
function upvoteComment(url, request) {
  ...
  if (!username || !savedComment || !database.users[username]) {
    response.status = 400
  }
  else if (savedComment.upvotedBy.indexOf(username) === -1) {
    savedComment.upvotedBy.push(username);
    downvotedIndex = savedComment.downvotedBy.indexOf(username);
    if (downvotedIndex != -1) {
      savedComment.downvotedBy.splice(downvotedIndex, 1);
    }
    response.status = 200;
...
function downvoteComment(url, request) {
  ...
  else if (savedComment.downvotedBy.indexOf(username) === -1) {
    savedComment.downvotedBy.push(username);
    upvotedIndex = savedComment.upvotedBy.indexOf(username);
    if (upvotedIndex != -1) {
      savedComment.upvotedBy.splice(upvotedIndex, 1);
    }
    response.status = 200;
    response.body = {comment: savedComment}
  }
...
function saveDatabase() {
  fs.writeFile('scoopDatabase.yml', yaml.dump(database));
  return null
}

function loadDatabase() {
  savedDatabase = yaml.load(fs.readFileSync('scoopDatabase.yml', 'utf8'))
